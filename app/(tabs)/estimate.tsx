...
export default function EstimateScreen() {
  ...
  const latestMergedJob = job ? mergedJobs.find((candidate) => candidate.sourceJobIds.includes(job.id)) : undefined;
  const latestMergedSourceJobs = latestMergedJob ? jobs.filter((candidate) => latestMergedJob.sourceJobIds.includes(candidate.id)) : [];
  const latestMergedUsage = latestMergedJob ? {
    materialLengthMm: latestMergedJob.usedLengthMm,
    productAreaM2: latestMergedSourceJobs.reduce((sum, source) => sum + Math.max(0, source.input.pieceWidthMm * source.input.pieceLengthMm * source.input.quantity) / 1_000_000, 0),
  } : undefined;
  const exportEstimatePdf = async () => {
    if (jobs.length === 0) return;
    const html = createEstimateHtml(jobs, mergedJobs, projectEstimate, materialCost, constructionCost, company);
...
const projectStyles = StyleSheet.create({
  card: { marginTop: 18, padding: 18, borderRadius: 16, backgroundColor: '#0f172a' }, eyebrow: { fontSize: 10, letterSpacing: 1.4, fontWeight: '800', color: '#93c5fd' }, title: { marginTop: 5, fontSize: 20, fontWeight: '800', color: '#fff' }, meta: { marginTop: 4, fontSize: 11, color: '#cbd5e1' }, total: { marginTop: 14, fontSize: 28, fontWeight: '900', color: '#bfdbfe' }, rows: { gap: 7, marginTop: 14, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#334155' }, row: { flexDirection: 'row', justifyContent: 'space-between' }, label: { fontSize: 11, color: '#cbd5e1' }, value: { fontSize: 11, fontWeight: '800', color: '#f8fafc' }, estimateActions: { flexDirection: 'row', gap: 8, marginTop: 14 }, copyButton: { minHeight: 42, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 13, borderRadius: 9, borderWidth: 1, borderColor: '#bfdbfe', backgroundColor: '#eff6ff' }, copyButtonText: { fontSize: 11, fontWeight: '800', color: '#1d4ed8' },
});
...
function createEstimateHtml(jobs: readonly SavedCuttingJob[], mergedJobs: readonly SavedMergedCuttingJob[], estimate: ProjectEstimate, materialCost: number, constructionCost: number, company: CompanyInfo): string {
  ...
  const sourceRows = jobs.filter((job) => !mergedSourceIds.has(job.id)).map((job) => `<tr><td>${esc(job.name)}</td><td>${esc(job.brand)} ${esc(job.productNumber)}</td><td>${job.result.newRollLengthMm.toLocaleString('ko-KR')}mm</td></tr>`);
  const mergedRows = mergedJobs.map((job) => `<tr><td>${esc(job.name)}</td><td>병합 ${esc(job.mergeGroupId)}</td><td>${job.usedLengthMm.toLocaleString('ko-KR')}mm</td></tr>`);
  const rows = [...sourceRows, ...mergedRows].join('');
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>필름 통합 견적서</title><style>body{font-family:sans-serif;padding:24px;color:#111827}table{border-collapse:collapse;width:100%;margin-top:16px}th,td{border:1px solid #cbd5e1;padding:8px;text-align:left}th{background:#f1f5f9}h1{margin-bottom:4px}.total{font-size:24px;font-weight:800;color:#1d4ed8}</style></head><body><h1>필름 통합 견적서</h1><p>${companyLine}</p><p class="total">총액 ${estimate.total.toLocaleString('ko-KR')}원</p><table><thead><tr><th>작업</th><th>제품</th><th>새 롤 길이</th></tr></thead><tbody>${rows}</tbody></table><table><tbody><tr><th>원단 단가</th><td>${materialCost.toLocaleString('ko-KR')}원/m</td></tr><tr><th>시공 단가</th><td>${constructionCost.toLocaleString('ko-KR')}원/m²</td></tr><tr><th>원단 비용</th><td>${estimate.materialCost.toLocaleString('ko-KR')}원</td></tr><tr><th>시공 비용</th><td>${estimate.constructionCost.toLocaleString('ko-KR')}원</td></tr><tr><th>할인</th><td>${Math.round(estimate.discountRate * 100)}% · -${estimate.discount.toLocaleString('ko-KR')}원</td></tr><tr><th>소계</th><td>${estimate.subtotal.toLocaleString('ko-KR')}원</td></tr></tbody></table><p>${esc(company.note)}</p></body></html>`;
}
function createEstimateText(jobs: readonly SavedCuttingJob[], estimate: ProjectEstimate, company: CompanyInfo): string { const contact = [company.companyName, company.managerName, company.phone, company.email].filter(Boolean).join(' · '); return `[필름 통합 견적서]\n${contact}\n\n작업 ${estimate.jobCount}건\n원단 비용: ${estimate.materialCost.toLocaleString('ko-KR')}원\n시공 비용: ${estimate.constructionCost.toLocaleString('ko-KR')}원\n할인: -${estimate.discount.toLocaleString('ko-KR')}원\n총액: ${estimate.total.toLocaleString('ko-KR')}원`; }
