...
export default function ProjectsScreen() {
  ...
  const saveCurrentEstimate = async () => {
    ...
    try {
      ...
      const normalizedName = name.toLocaleLowerCase('ko-KR');
      if (projects.some((project) => project.name.trim().toLocaleLowerCase('ko-KR') === normalizedName)) throw new Error('같은 이름의 프로젝트가 이미 있습니다. 다른 이름을 입력해 주세요.');
      const bundle = createProjectFromCurrentEstimate(name, snapshot, new Date().toISOString(), projects.map((project) => project.id));
      await repository.saveProjectBundle(bundle.project, bundle.jobs, bundle.mergedJobs);
      setNewProjectName('');
      await refresh();
      setNotice(`현재 재단 결과를 "${name}" 프로젝트로 저장했습니다.`);
      router.push({ pathname: '/input', params: { projectId: bundle.project.id } });
  ...
  return <ScrollView style={styles.page} contentContainerStyle={styles.content}>
    ...
    <View style={styles.projectActions}><TouchableOpacity accessibilityRole="button" accessibilityLabel="현재 재단 결과 프로젝트 저장" onPress={() => void saveCurrentEstimate()} disabled={busy} style={[styles.actionPrimary, busy && styles.disabled]}><Text style={styles.actionPrimaryText}>현재 재단 결과 저장</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel="프로젝트 백업 내보내기" onPress={() => void exportLibrary()} disabled={busy} style={[styles.actionSecondary, busy && styles.disabled]}><Text style={styles.actionSecondaryText}>백업 내보내기</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel="프로젝트 백업 불러오기" onPress={() => void importLibrary()} disabled={busy} style={[styles.actionSecondary, busy && styles.disabled]}><Text style={styles.actionSecondaryText}>백업 불러오기</Text></TouchableOpacity></View>
