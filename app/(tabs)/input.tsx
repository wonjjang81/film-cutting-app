...
import { PIECE_INPUT_UNIT_HINT, commitSubgroupName, compactFieldAffixes, compactFieldLayout, flattenSubgroupCards, hasAssignedSubgroups, multiplyPieceQuantityBySiteCount, normalizeSubgroupNameDraft, normalizeSubgroupSiteCount, renameSubgroupPieceDrafts, subgroupCardStackIndex, subgroupGroupSelectLabel, subgroupPieceDisplayName, subgroupPieceNamePart, toggleAllSubgroupCards } from '../../src/features/library/subgroupCards';
...
function NumericField({ label, unit, value, onChange, integer = false, step = 1, min = 0, compact = false }: { label: string; unit: string; value: string; onChange(value: string): void; integer?: boolean; step?: number; min?: number; compact?: boolean }) {
  ...
  if (compact) {
    ...
    const fixedInputWidth = layout.inputWidth ?? 0;
    return <View style={[styles.compactField, layout.fixed && { flex: 0, width: fixedInputWidth + 58 }]}>{affixes.label ? <Text style={styles.compactFieldLabel}>{affixes.label}</Text> : null}<View style={[styles.compactInputWrap, layout.fixed && { flex: 0, width: fixedInputWidth + 36 }]}><TouchableOpacity accessibilityRole="button" accessibilityLabel={`${label} ${step}${unit} 감소`} onPress={() => adjust(-1)} style={styles.compactStepperButton}><Text style={styles.compactStepperText}>−</Text></TouchableOpacity><TextInput accessibilityLabel={`${label} ${unit}`} inputMode="decimal" keyboardType="numeric" selectTextOnFocus style={[styles.compactInput, layout.fixed && { flex: 0, width: fixedInputWidth }]} value={value} onChangeText={(text) => onChange(text.replace(integer ? /[^0-9]/g : /[^0-9.]/g, ''))} /><TouchableOpacity accessibilityRole="button" accessibilityLabel={`${label} ${step}${unit} 증가`} onPress={() => adjust(1)} style={styles.compactStepperButton}><Text style={styles.compactStepperText}>＋</Text></TouchableOpacity></View>{affixes.unit ? <Text style={styles.compactTrailingUnit}>{affixes.unit}</Text> : null}</View>;
  }
