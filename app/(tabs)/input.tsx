...
export default function FilmCutInputScreen() {
  ...
  const renameGroup = (id: string, name: string) => {
    ...
    const autoPattern = new RegExp(`^${escapeRegExp(currentGroup.name)}[-_]\\d+$`);
    setGroups((items) => items.map((group) => group.id === id ? {
      ...group,
      name: nextName,
      pieces: group.pieces.map((piece, index) => {
        // Keep the legacy convenience of renaming untouched default pieces,
        // while preserving any piece name the operator customized.
    ...
    if (id === activeGroupId) {
      ...
      if (activePiece && autoPattern.test(activePiece.id)) setActivePieceId(defaultPieceId(nextName, currentGroup.pieces.findIndex((piece) => piece.id === activePiece.id) + 1));
