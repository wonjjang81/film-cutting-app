  ...
  return (
    <ScrollView style={styles.page} contentContainerStyle={[styles.pageContent, width < 420 && styles.pageContentSmall]} keyboardShouldPersistTaps="handled" nestedScrollEnabled>
      ...
      <View style={[styles.workspace, wide && styles.workspaceWide]}>
          ...
          <GroupInputPanel groups={groups} activeGroupId={activeGroupId} brand={form.brand} productNumber={form.productNumber} onSelect={selectGroup} onAdd={addGroup} onRename={renameGroup} onDelete={deleteGroup} onMergeGroup={changeMergeGroup} onBrandChange={(brand) => updateActiveForm((current) => ({ ...current, brand }))} onProductNumberChange={(productNumber) => updateActiveForm((current) => ({ ...current, productNumber }))} onSettings={(id, patch) => setGroups((items) => items.map((group) => group.id === id ? { ...group, ...patch } : group))} />
...
function GroupInputPanel({ groups, activeGroupId, brand, productNumber, onSelect, onAdd, onRename, onDelete, onMergeGroup, onBrandChange, onProductNumberChange, onSettings }: { groups: CuttingGroupDraft[]; activeGroupId: string; brand: string; productNumber: string; onSelect(group: CuttingGroupDraft): void; onAdd(): void; onRename(id: string, name: string): void; onDelete(id: string): void; onMergeGroup(id: string, mergeGroupId: string | undefined): void; onBrandChange(value: string): void; onProductNumberChange(value: string): void; onSettings(id: string, patch: Partial<Pick<CuttingGroupDraft, 'filmName' | 'materialCostPerM' | 'constructionCostPerM2'>>): void }) {
...
function PieceInputPanel({ group, activePieceId, onSelect, onAdd, onDelete, onRename, onChangeForm }: { group: CuttingGroupDraft; activePieceId: string; onSelect(piece: CuttingPieceDraft): void; onAdd(): void; onDelete(id: string): void; onRename(pieceId: string, nextId: string): void; onChangeForm(pieceId: string, updater: React.SetStateAction<CuttingFormState>): void }) {
  ...
  return <View style={styles.pieceInputPanel}>
    <View style={styles.pieceInputHeader}><View><Text style={styles.pieceInputTitle}>{group.name} · 소그룹 조각 모음</Text><Text style={styles.pieceInputHint}>소그룹은 조각 모음입니다. 각 조각의 폭·길이·수량을 개별 입력합니다.</Text></View><TouchableOpacity accessibilityRole="button" accessibilityLabel="현재 소그룹에 조각 추가" onPress={onAdd} style={styles.addPieceButton}><Text style={styles.addPieceButtonText}>＋ 조각 추가</Text></TouchableOpacity></View>
    <View style={styles.pieceRows}>{group.pieces.map((piece, index) => <View key={piece.id} style={[styles.pieceRowCard, piece.id === activePieceId && styles.pieceRowCardActive]}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pieceRowContent}>
        {editingId === piece.id
...
const styles = StyleSheet.create({
  ...
  pieceInputPanel: { marginBottom: 15, padding: 12, borderRadius: 12, borderWidth: 1, borderColor: '#e2e8f0', backgroundColor: '#fff' }, pieceInputHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8 }, pieceInputTitle: { fontSize: 13, fontWeight: '800', color: '#334155' }, pieceInputHint: { marginTop: 3, fontSize: 10, color: '#64748b' }, addPieceButton: { minHeight: 34, justifyContent: 'center', paddingHorizontal: 10, borderRadius: 8, backgroundColor: '#0f766e' }, addPieceButtonText: { fontSize: 11, fontWeight: '800', color: '#fff' }, pieceRows: { gap: 6, marginTop: 10 }, pieceRowCard: { minHeight: 46, overflow: 'hidden', borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 8, backgroundColor: '#f8fafc' }, pieceRowCardActive: { borderColor: '#0f766e', backgroundColor: '#f0fdfa' }, pieceRowContent: { minHeight: 46, minWidth: '100%', flexDirection: 'row', alignItems: 'center' }, pieceRowMain: { width: 116, flexShrink: 0, minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 8 }, pieceIndex: { width: 20, height: 20, textAlign: 'center', borderRadius: 10, backgroundColor: '#e2e8f0', fontSize: 10, lineHeight: 20, fontWeight: '800', color: '#64748b' }, pieceIndexActive: { backgroundColor: '#0f766e', color: '#fff' }, pieceCopy: { flex: 1, minWidth: 0 }, pieceName: { fontSize: 11, fontWeight: '700', color: '#475569' }, pieceNameActive: { color: '#115e59' }, pieceMeta: { marginTop: 2, fontSize: 9, color: '#64748b' }, pieceEditButton: { width: 28, height: 38, flexShrink: 0, alignItems: 'center', justifyContent: 'center' }, pieceEditButtonText: { fontSize: 13, fontWeight: '800', color: '#0f766e' }, pieceEditRow: { flex: 1, minWidth: 0, minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 7 }, pieceEditPrefix: { minHeight: 34, justifyContent: 'center', paddingLeft: 2 }, pieceEditPrefixText: { fontSize: 12, fontWeight: '800', color: '#115e59' }, pieceIdInput: { flex: 1, minWidth: 55, minHeight: 34, paddingHorizontal: 8, borderWidth: 1, borderColor: '#5eead4', borderRadius: 7, backgroundColor: '#fff', fontSize: 12, color: '#0f172a' }, pieceEditAction: { minHeight: 34, justifyContent: 'center', paddingHorizontal: 8, borderRadius: 6, backgroundColor: '#0f766e' }, pieceEditActionText: { fontSize: 10, fontWeight: '800', color: '#fff' }, pieceEditCancel: { minHeight: 34, justifyContent: 'center', paddingHorizontal: 5 }, pieceEditCancelText: { fontSize: 10, color: '#64748b' }, pieceEditError: { width: '100%', paddingHorizontal: 9, paddingBottom: 6, fontSize: 10, color: '#b91c1c' }, pieceCardFields: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 3 }, compactField: { flexDirection: 'row', alignItems: 'center', flexShrink: 0, gap: 2 }, compactFieldLabel: { fontSize: 10, fontWeight: '800', color: '#64748b' }, compactInputWrap: { width: 82, height: 34, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 7, backgroundColor: '#fff' }, compactInput: { flex: 1, minWidth: 22, height: 32, paddingHorizontal: 1, fontSize: 12, fontWeight: '700', color: '#0f172a', textAlign: 'center' }, compactUnit: { paddingHorizontal: 1, fontSize: 9, fontWeight: '700', color: '#94a3b8' }, compactStepperButton: { width: 20, height: 30, alignItems: 'center', justifyContent: 'center', borderRadius: 5, backgroundColor: '#e2e8f0' }, compactStepperText: { fontSize: 15, lineHeight: 17, color: '#334155' }, pieceDelete: { width: 28, height: 44, flexShrink: 0, alignItems: 'center', justifyContent: 'center' }, pieceDeleteText: { fontSize: 18, color: '#ef4444' },
