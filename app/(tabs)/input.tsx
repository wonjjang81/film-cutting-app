  ...
  return (
    <ScrollView style={styles.page} contentContainerStyle={[styles.pageContent, width < 420 && styles.pageContentSmall]} keyboardShouldPersistTaps="handled" nestedScrollEnabled>
      ...
      <View style={styles.workspace}>
          ...
          <GroupInputPanel groups={groups} activeGroupId={activeGroupId} onSelect={selectGroup} onAdd={addGroup} onRename={renameGroup} onRenameId={renameGroupDisplayId} onDelete={deleteGroup} onMergeGroup={changeMergeGroup} onGroupBrandChange={(id, brand) => updateGroupIdentityFor(id, { brand })} onGroupProductNumberChange={(id, productNumber) => updateGroupIdentityFor(id, { productNumber })} onSettings={(id, patch) => setGroups((items) => items.map((group) => group.id === id ? { ...group, ...patch } : group))} />
...
function GroupInputPanel({ groups, activeGroupId, onSelect, onAdd, onRename, onRenameId, onDelete, onMergeGroup, onGroupBrandChange, onGroupProductNumberChange, onSettings }: { groups: CuttingGroupDraft[]; activeGroupId: string; onSelect(group: CuttingGroupDraft): void; onAdd(): void; onRename(id: string, name: string): void; onRenameId(id: string, value: string): void; onDelete(id: string): void; onMergeGroup(id: string, mergeGroupId: string | undefined): void; onGroupBrandChange(id: string, value: string): void; onGroupProductNumberChange(id: string, value: string): void; onSettings(id: string, patch: Partial<Pick<CuttingGroupDraft, 'filmName' | 'materialCostPerM' | 'constructionCostPerM2'>>): void }) {
...
function PieceInputPanel({ group, activePieceId, onSelect, onAdd, onDelete, onRename, onChangeForm }: { group: CuttingGroupDraft; activePieceId: string; onSelect(piece: CuttingPieceDraft): void; onAdd(): void; onDelete(id: string): void; onRename(pieceId: string, nextId: string): void; onChangeForm(pieceId: string, updater: React.SetStateAction<CuttingFormState>): void }) {
  ...
  return <View style={styles.pieceInputPanel}>
    <View style={styles.pieceInputHeader}><View><Text style={styles.pieceInputTitle}>{group.name}</Text><Text style={styles.pieceInputHint}>조각별 폭·길이·수량을 입력합니다.</Text></View></View>
    ...
    </View>)}</View><TouchableOpacity accessibilityRole="button" accessibilityLabel={`${group.name} 소그룹 추가`} onPress={onAdd} style={styles.addPieceBottomButton}><Text style={styles.addPieceButtonText}>＋ 소그룹 추가</Text></TouchableOpacity>
  </View>;
}
...
const styles = StyleSheet.create({
  groupRows: { gap: 6, marginTop: 10 }, groupRow: { minHeight: 46, flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 6, borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 9, backgroundColor: '#fff' }, groupRowActive: { borderColor: '#2563eb', backgroundColor: '#eff6ff' }, groupRowSelect: { flex: 1, minWidth: 48, minHeight: 42, flexDirection: 'row', alignItems: 'center', gap: 5 }, compactBrandField: { width: 82, flexGrow: 0, flexShrink: 0, zIndex: 30 }, compactSelectButton: { minHeight: 32, paddingHorizontal: 7, borderRadius: 6 }, groupProductInput: { width: 92, minWidth: 55, height: 32, flexShrink: 1, paddingHorizontal: 6, borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 6, fontSize: 10, fontWeight: '700', color: '#0f172a', backgroundColor: '#fff' }, groupRowAction: { width: 24, height: 38, flexShrink: 0, alignItems: 'center', justifyContent: 'center' }, addPieceBottomButton: { minHeight: 38, alignItems: 'center', justifyContent: 'center', marginTop: 8, borderRadius: 8, backgroundColor: '#0f766e' },
