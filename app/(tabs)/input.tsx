...
export default function FilmCutInputScreen() {
  ...
  const activeMergedJob = useMemo(
    () => activeMergedPlan
      ? library.mergedJobs.find((job) => job.mergeGroupId === activeMergedPlan.mergeGroupId && job.sourceJobIds.includes(draftJob?.id ?? ''))
      : undefined,
  ...
  const toggleMergedPlacementComplete = async (jobId: string, placementId: number) => {
    const current = library.mergedJobs.find((job) => job.id === jobId);
    if (!current) return;
    ...
    if (isComplete && !current.isCuttingComplete && current.sourceJobIds.some((sourceId) => library.jobs.some((job) => job.id === sourceId && job.isInventoryConfirmed))) {
      setError('병합 롤의 원본 조각 중 이미 재고 확정된 작업이 있어 병합 롤을 완료 처리할 수 없습니다.');
      return;
    }
    setBusy(true); setError(null);
    ...
    <ScrollView style={styles.page} contentContainerStyle={[styles.pageContent, width < 420 && styles.pageContentSmall]} keyboardShouldPersistTaps="handled" nestedScrollEnabled>
      ...
      <View style={[styles.workspace, wide && styles.workspaceWide]}>
        <View style={[styles.panel, wide && styles.inputPanelWide]}>
          ...
          <View style={styles.calculateButtonRow}><TouchableOpacity accessibilityRole="button" accessibilityLabel="현재 조각 자투리 우선 자동 배치 계산" disabled={busy} onPress={() => void calculate()} style={[styles.primaryButton, styles.calculateButtonMain, busy && styles.disabled]}><Text style={styles.primaryButtonText}>{busy ? '처리 중…' : '현재 조각 배치'}</Text><Text style={styles.arrow}>→</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel="전체 그룹 통합 배치 계산" disabled={busy} onPress={() => void calculateAllGroups()} style={[styles.batchButton, busy && styles.disabled]}><Text style={styles.batchButtonText}>전체 그룹 계산</Text></TouchableOpacity></View>
        </View>
        <View style={[styles.panel, styles.previewPanel]}><PanelHeading step="02" title="배치 미리보기" subtitle={activeMergedPlan ? '병합 롤 도면은 아래 통합 배치 결과에서 확인할 수 있습니다.' : (preview?.title ?? '계산 후 연속 롤 도면을 표시합니다.')} dark />{activeMergedPlan ? <View style={styles.mergedPreviewNotice}><Text style={styles.mergedPreviewNoticeTitle}>병합 롤 도면은 하단에서 확인</Text><Text style={styles.mergedPreviewNoticeText}>중복 표시를 줄이기 위해 병합 롤 도면과 완료 체크 목록은 아래 그룹 통합 배치 결과에 한 번만 표시합니다.</Text></View> : <FilmLayoutPreview result={previewResult} rollWidthMm={preview?.widthMm ?? Number(form.rollWidth)} sideMarginMm={preview?.sideMarginMm ?? Number(form.sideMargin)} startEndMarginMm={preview?.startEndMarginMm ?? Number(form.startEndMargin)} completedPlacementIds={checkedPlacementIds} />}</View>
      </View>
      ...
      {plan && draftJob && resultStatus && <View style={styles.resultSection} accessibilityRole="summary">
        ...
        {batchPlans && <BatchPlanSummary plans={batchPlans} mergedPlans={mergedGroupPlans} mergedJobs={library.mergedJobs} busy={busy} onConfirmBatch={() => void confirmBatch()} onToggleMergedComplete={(id) => void toggleMergedComplete(id)} onToggleMergedPlacementComplete={(jobId, placementId) => void toggleMergedPlacementComplete(jobId, placementId)} onConfirmMergedInventory={(id) => void confirmMergedInventory(id)} />}
...
function BatchPlanSummary({ plans, mergedPlans, mergedJobs, busy, onConfirmBatch, onToggleMergedComplete, onToggleMergedPlacementComplete, onConfirmMergedInventory }: { plans: readonly GroupedPiecePlan[]; mergedPlans: readonly MergedGroupPlan[]; mergedJobs: readonly SavedMergedCuttingJob[]; busy: boolean; onConfirmBatch(): void; onToggleMergedComplete(id: string): void; onToggleMergedPlacementComplete(jobId: string, placementId: number): void; onConfirmMergedInventory(id: string): void }) {
