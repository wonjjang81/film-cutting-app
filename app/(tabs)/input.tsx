  ...
  return (
    <ScrollView style={styles.page} contentContainerStyle={[styles.pageContent, width < 420 && styles.pageContentSmall]} keyboardShouldPersistTaps="handled" nestedScrollEnabled>
      ...
      <View style={styles.workspace}>
          ...
          <GroupInputPanel groups={groups} activeGroupId={activeGroupId} brand={form.brand} productNumber={form.productNumber} onSelect={selectGroup} onAdd={addGroup} onRename={renameGroup} onRenameId={renameGroupDisplayId} onDelete={deleteGroup} onMergeGroup={changeMergeGroup} onBrandChange={(brand) => updateActiveForm((current) => ({ ...current, brand }))} onProductNumberChange={(productNumber) => updateActiveForm((current) => ({ ...current, productNumber }))} onSettings={(id, patch) => setGroups((items) => items.map((group) => group.id === id ? { ...group, ...patch } : group))} />
          <PieceInputPanel group={groups.find((group) => group.id === activeGroupId)!} groupOptions={groups.map((group) => ({ id: group.id, displayId: group.displayId, name: group.name }))} activePieceId={activePieceId} onSelect={selectPiece} onMove={(pieceId, targetDisplayId) => movePieceToGroup(activeGroupId, pieceId, targetDisplayId)} onAdd={addPiece} onDelete={deletePiece} onRename={(pieceId, nextId) => renamePieceId(activeGroupId, pieceId, nextId)} onChangeForm={(pieceId, updater) => updatePieceForm(activeGroupId, pieceId, updater)} />
...
function GroupInputPanel({ groups, activeGroupId, brand, productNumber, onSelect, onAdd, onRename, onRenameId, onDelete, onMergeGroup, onBrandChange, onProductNumberChange, onSettings }: { groups: CuttingGroupDraft[]; activeGroupId: string; brand: string; productNumber: string; onSelect(group: CuttingGroupDraft): void; onAdd(): void; onRename(id: string, name: string): void; onRenameId(id: string, value: string): void; onDelete(id: string): void; onMergeGroup(id: string, mergeGroupId: string | undefined): void; onBrandChange(value: string): void; onProductNumberChange(value: string): void; onSettings(id: string, patch: Partial<Pick<CuttingGroupDraft, 'filmName' | 'materialCostPerM' | 'constructionCostPerM2'>>): void }) {
...
function PieceInputPanel({ group, groupOptions, activePieceId, onSelect, onMove, onAdd, onDelete, onRename, onChangeForm }: { group: CuttingGroupDraft; groupOptions: { id: string; displayId: string; name: string }[]; activePieceId: string; onSelect(piece: CuttingPieceDraft): void; onMove(pieceId: string, targetDisplayId: string): void; onAdd(): void; onDelete(id: string): void; onRename(pieceId: string, nextId: string): void; onChangeForm(pieceId: string, updater: React.SetStateAction<CuttingFormState>): void }) {
  const [editingId, setEditingId] = useState<string | null>(null);
...
function PieceGroupIdInput({ pieceId, value, options, onCommit }: { pieceId: string; value: string; options: { id: string; displayId: string; name: string }[]; onCommit(value: string): void }) {
  const [draft, setDraft] = useState(value);
  useEffect(() => setDraft(value), [value]);
  const commit = () => { const next = draft.replace(/[^0-9]/g, ''); setDraft(value); if (next && options.some((option) => option.displayId === next)) onCommit(next); };
  return <TextInput accessibilityLabel={`${pieceId} 대그룹 ID`} value={draft} onChangeText={(next) => setDraft(next.replace(/[^0-9]/g, ''))} onBlur={commit} onSubmitEditing={commit} keyboardType="numeric" style={styles.pieceGroupIdInput} />;
}
