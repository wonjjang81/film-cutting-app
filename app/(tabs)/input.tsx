...
import { Alert, Platform, ScrollView, StyleSheet, Switch, Text, TextInput, TouchableOpacity, useWindowDimensions, View } from 'react-native';
...
import { compactBrandSelectWidth, DEFAULT_BRANDS, isDefaultBrand, normalizeBrandList } from '../../src/features/library/brandOptions';
...
import { applyFixedProductionDefaults, DEFAULT_GAP_MM, DEFAULT_SIDE_MARGIN_MM, DEFAULT_START_END_MARGIN_MM, FIXED_ROLL_WIDTH_MM, normalizeProductionMargin } from '../../src/features/cutting/productionConditions';
...
export default function FilmCutInputScreen() {
  ...
  const updateProductionMargin = (field: 'sideMargin' | 'startEndMargin', value: string) => {
    setGroups((items) => items.map((group) => ({
      ...group,
      form: { ...group.form, [field]: value },
      pieces: group.pieces.map((piece) => ({ ...piece, form: { ...piece.form, [field]: value } })),
    })));
    setForm((current) => ({ ...current, [field]: value }));
    setPlan(null); setPlanRequest(null); setDraftJob(null); setBatchPlans(null); setMergedGroupPlans([]); setPendingBatchSave(null); setSavedGroupPlanViews({}); setManualPlacements(null); setCheckedPlacementIds([]); setConfirmed(false); setCuttingComplete(false);
  };
  const clearCalculatedAllowanceResults = () => {
    setPlan(null); setPlanRequest(null); setDraftJob(null); setBatchPlans(null); setMergedGroupPlans([]); setPendingBatchSave(null); setSavedGroupPlanViews({}); setManualPlacements(null); setCheckedPlacementIds([]); setConfirmed(false); setCuttingComplete(false);
  };
  const applyAllCutAllowance = (value: string) => {
    const normalized = String(normalizeCutAllowance(value));
    setGroups((items) => items.map((group) => ({
      ...group,
      form: { ...group.form, cutAllowance: normalized },
      pieces: group.pieces.map((piece) => ({ ...piece, form: { ...piece.form, cutAllowance: normalized } })),
    })));
    setForm((current) => ({ ...current, cutAllowance: normalized }));
    clearCalculatedAllowanceResults();
    setError(null); setNotice(`전체 조각의 재단 여유치를 ${normalized}mm로 변경했습니다.`);
  };
  const applySubgroupCutAllowance = (groupId: string, subgroupId: string, value: string) => {
    const normalized = String(normalizeCutAllowance(value));
    const subgroup = groups.find((group) => group.id === groupId)?.subgroups.find((item) => item.id === subgroupId);
    if (!subgroup) return;
    const pieceIds = new Set(subgroup.pieceIds);
    setGroups((items) => items.map((group) => group.id !== groupId ? group : {
      ...group,
      form: pieceIds.has(activePieceId) ? { ...group.form, cutAllowance: normalized } : group.form,
      pieces: group.pieces.map((piece) => pieceIds.has(piece.id) ? { ...piece, form: { ...piece.form, cutAllowance: normalized } } : piece),
    }));
    if (groupId === activeGroupId && pieceIds.has(activePieceId)) setForm((current) => ({ ...current, cutAllowance: normalized }));
    clearCalculatedAllowanceResults();
    setError(null); setNotice(`소그룹 ${subgroup.name}의 재단 여유치를 ${normalized}mm로 변경했습니다.`);
  };
  ...
  const addPiece = (subgroupId?: string, groupId = activeGroupId, overallAllowance: unknown = DEFAULT_CUT_ALLOWANCE_MM) => {
    ...
    const targetSubgroup = group.subgroups.find((subgroup) => subgroup.id === subgroupId)
      ...
      ?? group.subgroups[0];
    if (!targetSubgroup) { addSubgroup(overallAllowance); return; }
    const next = newPieceDraft(`${group.name}_${targetSubgroup.name}`, targetSubgroup.pieceIds.length + 1);
    next.form = { ...inheritCutAllowance(next.form, overallAllowance), sideMargin: form.sideMargin, startEndMargin: form.startEndMargin };
  ...
  const addSubgroup = (overallAllowance: unknown = DEFAULT_CUT_ALLOWANCE_MM) => {
    ...
    const pieces = [1, 2, 3].map((index) => {
      const piece = newPieceDraft(`${group.name}_${subgroupName}`, index);
      return { ...piece, form: { ...inheritCutAllowance(piece.form, overallAllowance), sideMargin: form.sideMargin, startEndMargin: form.startEndMargin } };
    });
  ...
  const addGroup = () => {
    ...
    next.form = { ...next.form, sideMargin: form.sideMargin, startEndMargin: form.startEndMargin };
  ...
  const loadPreset = (preset: FilmPreset) => {
    const restored = restoreCutAllowanceDimensions(preset);
    return void calculate({
      brand: preset.brand, productNumber: preset.productNumber, rollWidth: String(FIXED_ROLL_WIDTH_MM),
      pieceWidth: String(restored.sourcePieceWidthMm), pieceLength: String(restored.sourcePieceLengthMm), cutAllowance: String(restored.cutAllowanceMm), quantity: form.quantity,
      gap: String(DEFAULT_GAP_MM), sideMargin: String(preset.sideMarginMm), startEndMargin: String(preset.startEndMarginMm), allowRotation: true,
    });
  };
  const loadJob = (job: SavedCuttingJob) => void calculate(formFromSavedJob(job), useRemnants, Boolean(job.isCuttingComplete), job.completedPlacementIds ?? [], job.id);
  useEffect(() => {
...
function GroupInputPanel({ groups, activeGroupId, onSelect, onAdd, onRenameId, onDelete, onPatternFixedChange, onGroupBrandChange, onGroupProductNumberChange }: { groups: CuttingGroupDraft[]; activeGroupId: string; onSelect(group: CuttingGroupDraft): void; onAdd(): void; onRenameId(id: string, value: string): void; onDelete(id: string): void; onPatternFixedChange(id: string, value: boolean): void; onGroupBrandChange(id: string, value: string): void; onGroupProductNumberChange(id: string, value: string): void }) {
  ...
  return <View style={[styles.groupInputPanel, styles.overlayGroupPanel]}>
    ...
    <View style={styles.groupRows}>{groups.map((group, index) => { const baseLayer = subgroupCardStackIndex(groups.length, index); const rowLayer = openBrandGroupId === group.id ? groups.length + 10 : baseLayer; return <View key={group.id} style={[styles.groupRow, styles.groupRowWrap, { zIndex: rowLayer, elevation: rowLayer }, group.id === activeGroupId && styles.groupRowActive]}>
      <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${group.name} 선택`} accessibilityState={{ selected: group.id === activeGroupId }} onPress={() => onSelect(group)} style={[styles.groupRowSelect, styles.groupRowSelectFitted]}>
      ...
      <TextInput accessibilityLabel={`${group.name} 제품 번호`} autoCapitalize="none" value={group.form.productNumber} onChangeText={(value) => onGroupProductNumberChange(group.id, value)} placeholder="제품번호" placeholderTextColor="#94a3b8" style={[styles.groupProductInput, { width: Math.max(88, Math.min(180, 20 + Array.from(group.form.productNumber).reduce((sum, character) => sum + (/[\uAC00-\uD7A3]/.test(character) ? 12 : 7), 0))) }]} />
...
function PieceInputPanel({ groups, groupOptions, activePieceId, onSelect, onAdd, onAddSubgroup, onDelete, onRename, onRenameSubgroup, onChangeDifficulty, onChangeSiteCount, onChangeOverallDimensions, onMoveSubgroup, onChangeForm, onApplyAllAllowance, onApplySubgroupAllowance }: { groups: CuttingGroupDraft[]; groupOptions: { id: string; displayId: string }[]; activePieceId: string; onSelect(groupId: string, piece: CuttingPieceDraft): void; onAdd(groupId: string, subgroupId: string | undefined, overallAllowance: string): void; onAddSubgroup(overallAllowance: string): void; onDelete(groupId: string, id: string): void; onRename(groupId: string, pieceId: string, nextId: string): void; onRenameSubgroup(groupId: string, subgroupId: string, name: string): void; onChangeDifficulty(groupId: string, subgroupId: string, difficulty: ConstructionDifficulty): void; onChangeSiteCount(groupId: string, subgroupId: string, value: string): void; onChangeOverallDimensions(groupId: string, subgroupId: string, field: keyof EditableSubgroupOverallDimensions, value: string): void; onMoveSubgroup(sourceGroupId: string, subgroupId: string, targetGroupId: string): void; onChangeForm(groupId: string, pieceId: string, updater: React.SetStateAction<CuttingFormState>): void; onApplyAllAllowance(value: string): void; onApplySubgroupAllowance(groupId: string, subgroupId: string, value: string): void }) {
  ...
  const confirmApply = (message: string, action: () => void) => {
    ...
    Alert.alert('재단 여유치 일괄 수정', message, [{ text: '취소', style: 'cancel' }, { text: '적용', style: 'destructive', onPress: action }]);
  };
  ...
  const renderPiece = (group: CuttingGroupDraft, subgroupName: string, piece: CuttingPieceDraft, index: number) => <View key={piece.id} style={[styles.pieceRowCard, piece.id === activePieceId && styles.pieceRowCardActive]}>
    <View style={[styles.pieceRowContent, narrowInput && styles.pieceRowContentNarrow]}>
      {editingId === piece.id
        ? <View style={styles.pieceEditRow}><View style={styles.pieceEditPrefix}><Text style={styles.pieceEditPrefixText}>{group.name}_</Text></View><TextInput accessibilityLabel={`${piece.id} 조각 이름`} autoFocus value={editingValue} onChangeText={(value) => { setEditingValue(value.replace(/[\s_]+/g, '')); setEditingError(null); }} onSubmitEditing={() => finishRename(group.id, piece.id)} returnKeyType="done" style={styles.pieceIdInput} /><TouchableOpacity accessibilityLabel={`${piece.id} 조각 이름 저장`} onPress={() => finishRename(group.id, piece.id)} style={styles.pieceEditAction}><Text style={styles.pieceEditActionText}>저장</Text></TouchableOpacity><TouchableOpacity accessibilityLabel="조각 이름 변경 취소" onPress={() => { setEditingId(null); setEditingError(null); }} style={styles.pieceEditCancel}><Text style={styles.pieceEditCancelText}>취소</Text></TouchableOpacity></View>
         : <><TouchableOpacity accessibilityRole="button" accessibilityLabel={`${piece.id} 선택`} accessibilityState={{ selected: piece.id === activePieceId }} onPress={() => onSelect(group.id, piece)} style={[styles.pieceRowMain, styles.compactPieceRowMain]}><Text style={[styles.pieceIndex, piece.id === activePieceId && styles.pieceIndexActive]}>{index + 1}</Text><View style={styles.pieceCopy}><Text style={[styles.pieceName, piece.id === activePieceId && styles.pieceNameActive]} numberOfLines={1}>{subgroupPieceDisplayName(group.name, subgroupName, piece.id)}</Text></View></TouchableOpacity><TouchableOpacity accessibilityLabel={`${piece.id} 조각 이름 변경`} onPress={() => { setEditingId(piece.id); setEditingValue(subgroupPieceNamePart(group.name, subgroupName, piece.id)); setEditingError(null); }} style={styles.pieceEditButton}><Text style={styles.pieceEditButtonText}>✎</Text></TouchableOpacity><TouchableOpacity accessibilityLabel={`${piece.id} 재단 여유치 설정`} onPress={() => { setAllowanceEditorId((current) => current === piece.id ? null : piece.id); setAllowanceDrafts((current) => ({ ...current, [piece.id]: piece.form.cutAllowance ?? '0' })); }} style={styles.allowanceBadge}><Text style={styles.allowanceBadgeText}>+{piece.form.cutAllowance ?? '0'}</Text></TouchableOpacity>{group.pieces.length > 1 && <TouchableOpacity accessibilityLabel={`${piece.id} 삭제`} onPress={() => onDelete(group.id, piece.id)} style={styles.pieceDelete}><Text style={styles.pieceDeleteText}>×</Text></TouchableOpacity>}<View style={[styles.pieceCardFields, styles.expandedPieceCardFields, narrowInput && styles.pieceCardFieldsNarrow]}><NumericField compact label="재단 폭" unit="mm" value={piece.form.pieceWidth} step={50} min={0} onChange={(value) => onChangeForm(group.id, piece.id, (current) => ({ ...current, pieceWidth: value }))} /><NumericField compact label="재단 길이" unit="mm" value={piece.form.pieceLength} step={50} min={0} onChange={(value) => onChangeForm(group.id, piece.id, (current) => ({ ...current, pieceLength: value }))} /><NumericField compact label="필요 수량" unit="개" value={piece.form.quantity} integer step={1} min={1} onChange={(value) => onChangeForm(group.id, piece.id, (current) => ({ ...current, quantity: value }))} /></View></>}
    </View>
    {editingError && editingId === piece.id && <Text style={styles.pieceEditError}>{editingError}</Text>}
    {allowanceEditorId === piece.id && <View style={styles.allowanceEditor}><Text style={styles.allowanceEditorLabel}>조각 재단 여유치</Text><NumericField compact label="재단 여유치" unit="mm" value={allowanceDrafts[piece.id] ?? piece.form.cutAllowance ?? '0'} step={10} min={0} onChange={(value) => setAllowanceDrafts((current) => ({ ...current, [piece.id]: value }))} /><TouchableOpacity accessibilityRole="button" accessibilityLabel={`${piece.id} 재단 여유치 저장`} onPress={() => { const value = String(normalizeCutAllowance(allowanceDrafts[piece.id] ?? piece.form.cutAllowance)); onChangeForm(group.id, piece.id, (current) => ({ ...current, cutAllowance: value })); setAllowanceEditorId(null); }} style={styles.allowanceApplyButton}><Text style={styles.allowanceApplyButtonText}>적용</Text></TouchableOpacity></View>}
  </View>;
  const subgroupCards = flattenSubgroupCards(groups);
  const subgroupIds = subgroupCards.map(({ subgroup }) => subgroup.id);
  const allSubgroupsExpanded = subgroupIds.length > 0 && subgroupIds.every((id) => collapsedSubgroups[id] !== true);
  return <View style={[styles.pieceInputPanel, styles.overlayPiecePanel]}>
    <View style={[styles.pieceInputHeader, styles.wrappingHeader]}><View style={[styles.pieceHeaderCopy, styles.headerCopyFlexible]}><Text style={styles.pieceInputTitle}>소그룹 입력</Text><Text style={styles.pieceInputUnitHint}>{PIECE_INPUT_UNIT_HINT}</Text><Text style={styles.pieceInputHint}>소그룹 카드는 독립적으로 관리하며, 각 카드에서 대그룹 ID를 지정합니다.</Text></View><View style={[styles.pieceHeaderActions, styles.headerActionsWrap]}><TouchableOpacity accessibilityRole="button" accessibilityLabel="새 소그룹 추가" onPress={() => onAddSubgroup(globalAllowance)} style={styles.addSubgroupButton}><Text style={styles.addSubgroupButtonText}>＋ 소그룹 추가</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel={subgroupIds.length === 0 ? '소그룹 없음' : allSubgroupsExpanded ? '소그룹 모두 접기' : '소그룹 모두 펼치기'} disabled={subgroupIds.length === 0} onPress={() => setCollapsedSubgroups((current) => toggleAllSubgroupCards(subgroupIds, current))} style={[styles.collapseButton, subgroupIds.length === 0 && styles.disabled]}><Text style={styles.collapseButtonText}>{allSubgroupsExpanded ? '모두 접기' : '모두 펼치기'}</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel={expanded ? '소그룹 입력 접기' : '소그룹 입력 펼치기'} onPress={() => setExpanded((current) => !current)} style={styles.collapseButton}><Text style={styles.collapseButtonText}>{expanded ? '접기' : '펼치기'}</Text></TouchableOpacity></View></View>
    <View style={[styles.globalAllowanceRow, styles.wrappingHeader]}><Text style={styles.allowanceEditorLabel}>전체 여유치</Text><NumericField compact label="재단 여유치" unit="mm" value={globalAllowance} step={10} min={0} onChange={setGlobalAllowance} /><TouchableOpacity accessibilityRole="button" accessibilityLabel="전체 조각 재단 여유치 적용" onPress={() => confirmApply(`현재 ${groups.reduce((sum, group) => sum + group.pieces.length, 0)}개 조각의 여유치를 ${globalAllowance || '0'}mm로 변경할까요?`, () => onApplyAllAllowance(globalAllowance))} style={styles.allowanceApplyButton}><Text style={styles.allowanceApplyButtonText}>전체 적용</Text></TouchableOpacity></View>
    {expanded && (subgroupCards.length === 0
      ...
      : subgroupCards.map(({ groupId, subgroup }, subgroupIndex) => {
        const group = groups.find((item) => item.id === groupId);
        ...
        const totalPieceQuantity = subgroupTotalPieceQuantity(pieces.map((piece) => piece.form.quantity), subgroup.siteCount);
...
function NumericField({ label, unit, value, onChange, integer = false, step = 1, min = 0, compact = false }: { label: string; unit: string; value: string; onChange(value: string): void; integer?: boolean; step?: number; min?: number; compact?: boolean }) {
  ...
  if (compact) {
    const affixes = compactFieldAffixes(label, unit);
    const layout = compactFieldLayout(label);
    return <View style={[styles.compactField, layout?.field]}>{affixes.label ? <Text style={styles.compactFieldLabel}>{affixes.label}</Text> : null}<View style={[styles.compactInputWrap, layout?.inputWrap]}><TouchableOpacity accessibilityRole="button" accessibilityLabel={`${label} ${step}${unit} 감소`} onPress={() => adjust(-1)} style={styles.compactStepperButton}><Text style={styles.compactStepperText}>−</Text></TouchableOpacity><TextInput accessibilityLabel={`${label} ${unit}`} inputMode="decimal" keyboardType="numeric" selectTextOnFocus style={[styles.compactInput, layout?.input]} value={value} onChangeText={(text) => onChange(text.replace(integer ? /[^0-9]/g : /[^0-9.]/g, ''))} /><TouchableOpacity accessibilityRole="button" accessibilityLabel={`${label} ${step}${unit} 증가`} onPress={() => adjust(1)} style={styles.compactStepperButton}><Text style={styles.compactStepperText}>＋</Text></TouchableOpacity></View>{affixes.unit ? <Text style={styles.compactTrailingUnit}>{affixes.unit}</Text> : null}</View>;
  }
...
function BrandSelect({ value, onChange, compact = false, onOpenChange }: { value: string; onChange(value: string): void; compact?: boolean; onOpenChange?: (open: boolean) => void }) {
  ...
  const compactWidth = compactBrandSelectWidth(value);
  ...
  return <View style={[styles.brandField, compact && styles.compactBrandField, compact && { width: compactWidth }]}>
...
function EditableProductionConditions({ sideMargin, startEndMargin, disabled, onChangeSideMargin, onChangeStartEndMargin }: { sideMargin: string; startEndMargin: string; disabled: boolean; onChangeSideMargin(value: string): void; onChangeStartEndMargin(value: string): void }) {
  const updateNumeric = (value: string, onChange: (next: string) => void) => { if (/^\d*(?:\.\d*)?$/.test(value)) onChange(value); };
  const normalize = (value: string, onChange: (next: string) => void) => onChange(String(normalizeProductionMargin(value)));
  return <View style={styles.fixedConditions}>
    <View style={styles.fixedConditionHeader}><Text style={styles.groupTitle}>재단 조건</Text><Text style={styles.editableBadge}>설정 가능</Text></View>
    <Text style={styles.fixedConditionText}>원본 롤 폭 {FIXED_ROLL_WIDTH_MM.toLocaleString()} mm · 길이 제한 없음 · 간격 {DEFAULT_GAP_MM} mm</Text>
    <View style={styles.marginSettingsRow}>
      <View style={styles.marginSettingField}><Text style={styles.marginSettingLabel}>좌우 여백</Text><View style={styles.marginSettingInputWrap}><TextInput accessibilityLabel="좌우 여백 mm" value={sideMargin} editable={!disabled} selectTextOnFocus keyboardType="decimal-pad" onChangeText={(value) => updateNumeric(value, onChangeSideMargin)} onBlur={() => normalize(sideMargin, onChangeSideMargin)} style={styles.marginSettingInput} /><Text style={styles.marginSettingUnit}>mm</Text></View></View>
      <View style={styles.marginSettingField}><Text style={styles.marginSettingLabel}>상하 여백</Text><View style={styles.marginSettingInputWrap}><TextInput accessibilityLabel="상하 여백 mm" value={startEndMargin} editable={!disabled} selectTextOnFocus keyboardType="decimal-pad" onChangeText={(value) => updateNumeric(value, onChangeStartEndMargin)} onBlur={() => normalize(startEndMargin, onChangeStartEndMargin)} style={styles.marginSettingInput} /><Text style={styles.marginSettingUnit}>mm</Text></View></View>
    </View>
    <Text style={styles.marginSettingHint}>입력한 여백은 모든 조각의 배치와 원단 길이 계산에 적용됩니다.</Text>
  </View>;
}
function ProductionSettingsCard({ sideMargin, startEndMargin, useRemnants, autoSaveHistory, busy, onChangeSideMargin, onChangeStartEndMargin, onToggleRemnants, onToggleHistory }: { sideMargin: string; startEndMargin: string; useRemnants: boolean; autoSaveHistory: boolean; busy: boolean; onChangeSideMargin(value: string): void; onChangeStartEndMargin(value: string): void; onToggleRemnants(value: boolean): void; onToggleHistory(value: boolean): void }) {
  const [expanded, setExpanded] = useState(false);
  return <View style={styles.productionSettingsCard}><TouchableOpacity accessibilityRole="button" accessibilityLabel={expanded ? '생산 설정 접기' : '생산 설정 펼치기'} accessibilityState={{ expanded }} onPress={() => setExpanded((current) => !current)} style={styles.productionSettingsHeader}><View><Text style={styles.productionSettingsTitle}>생산 설정</Text><Text style={styles.productionSettingsHint}>재단 조건 · 자투리 사용 · 작업이력 자동저장</Text></View><Text style={styles.productionSettingsToggle}>{expanded ? '⌃' : '⌄'}</Text></TouchableOpacity>{expanded && <View style={styles.productionSettingsBody}><EditableProductionConditions sideMargin={sideMargin} startEndMargin={startEndMargin} disabled={busy} onChangeSideMargin={onChangeSideMargin} onChangeStartEndMargin={onChangeStartEndMargin} /><View style={styles.switchCard}><View style={styles.switchCopy}><Text style={styles.switchTitle}>자투리 사용</Text><Text style={styles.switchDescription}>{useRemnants ? '브랜드 기준으로 사용 가능한 자투리를 먼저 배치합니다.' : '새 원본 롤만 계산합니다. 필요할 때 켜 주세요.'}</Text></View><Switch accessibilityLabel="자투리 사용" value={useRemnants} disabled={busy} onValueChange={onToggleRemnants} trackColor={{ false: '#cbd5e1', true: '#99f6e4' }} thumbColor={useRemnants ? '#0f766e' : '#f8fafc'} /></View><View style={styles.historySwitchCard}><View style={styles.switchCopy}><Text style={styles.historySwitchTitle}>작업이력 자동저장</Text><Text style={styles.historySwitchDescription}>{autoSaveHistory ? '계산할 때마다 작업 이력에 자동 저장합니다.' : '기본 OFF: 계산 결과는 임시 상태로만 유지합니다. 프로젝트 저장을 눌러야 이력에 남습니다.'}</Text></View><Switch accessibilityLabel="작업이력 자동저장" value={autoSaveHistory} disabled={busy} onValueChange={onToggleHistory} trackColor={{ false: '#cbd5e1', true: '#93c5fd' }} thumbColor={autoSaveHistory ? '#2563eb' : '#f8fafc'} /></View></View>}</View>;
}
...
function withProductionDefaults(form: CuttingFormState): CuttingFormState {
  ...
  return applyFixedProductionDefaults({ ...form, brand });
}
...
const styles = StyleSheet.create({
  ...
  groupRows: { gap: 6, marginTop: 10, overflow: 'visible' }, groupRow: { position: 'relative', overflow: 'visible', minHeight: 46, flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 6, borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 9, backgroundColor: '#fff' }, groupRowActive: { borderColor: '#2563eb', backgroundColor: '#eff6ff' }, groupRowSelect: { flex: 1, minWidth: 48, minHeight: 42, flexDirection: 'row', alignItems: 'center', gap: 5 }, compactBrandField: { flexGrow: 0, flexShrink: 0, position: 'relative', zIndex: 30 }, compactSelectButton: { minHeight: 32, paddingHorizontal: 7, borderRadius: 6 }, compactOptionList: { position: 'absolute', top: 38, left: 0, right: 'auto', width: 190, marginTop: 0, zIndex: 100 }, groupProductInput: { width: 130, minWidth: 88, height: 32, flexShrink: 1, paddingHorizontal: 8, borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 6, fontSize: 10, fontWeight: '700', color: '#0f172a', backgroundColor: '#fff' }, patternFixedControl: { width: 58, flexShrink: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: 2 }, patternFixedLabel: { fontSize: 9, fontWeight: '800', color: '#475569' }, groupRowAction: { width: 24, height: 38, flexShrink: 0, alignItems: 'center', justifyContent: 'center' }, addPieceBottomButton: { minHeight: 38, alignItems: 'center', justifyContent: 'center', marginTop: 8, borderRadius: 8, backgroundColor: '#0f766e' },
  ...
  groupRowWrap: { flexWrap: 'wrap', paddingVertical: 3 },
  groupRowSelectFitted: { minWidth: 108 },
  wrappingHeader: { flexWrap: 'wrap' },
  headerCopyFlexible: { flexGrow: 1, flexShrink: 1, minWidth: 130 },
  headerActionsWrap: { flexWrap: 'wrap' },
  subgroupHeaderWrap: { flexWrap: 'wrap' },
  subgroupNameInputFitted: { flexGrow: 0, flexShrink: 1, flexBasis: 'auto' },
  pieceRowContentNarrow: { flexWrap: 'wrap' },
  pieceCardFieldsNarrow: { flexGrow: 0, flexShrink: 0, flexBasis: '100%', width: '100%', minHeight: 38, paddingHorizontal: 6, paddingBottom: 5 },
  compactTrailingUnit: { flexShrink: 0, fontSize: 9, fontWeight: '800', color: '#64748b' },
