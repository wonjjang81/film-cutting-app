  ...
  return (
    <ScrollView style={styles.page} contentContainerStyle={[styles.pageContent, width < 420 && styles.pageContentSmall]} keyboardShouldPersistTaps="handled" nestedScrollEnabled>
      ...
      {plan && draftJob && resultStatus && <View style={styles.resultSection} accessibilityRole="summary">
        ...
        <View style={styles.resultMovedNotice}><Text style={styles.resultMovedTitle}>배치 결과는 배치계획에서 확인합니다.</Text><Text style={styles.resultMovedText}>중복 도면과 배치목록을 제거하고, 배치계획 탭에서 통합 결과를 표시합니다.</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="배치계획으로 이동" onPress={() => router.push('/planning')} style={styles.resultMovedButton}><Text style={styles.resultMovedButtonText}>배치계획 열기 →</Text></TouchableOpacity></View>
        {batchPlans && <TouchableOpacity accessibilityRole="button" accessibilityLabel="배치계획에서 통합 결과 보기" onPress={() => router.push('/planning')} style={styles.resultBatchLink}><Text style={styles.resultBatchLinkText}>현재 계산된 {batchPlans.length}개 조각의 통합 결과를 배치계획에서 확인할 수 있습니다.</Text></TouchableOpacity>}
        <View>
