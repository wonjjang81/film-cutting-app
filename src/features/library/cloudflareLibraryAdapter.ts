...
export function createCloudflareLibraryAdapter({ baseUrl, fetchImpl = fetch }: CloudflareLibraryAdapterOptions): KeyValueAdapter {
  ...
  return {
    async get(): Promise<string | null> {
      const response = await fetchImpl(endpoint(baseUrl), { credentials: 'include', headers: { Accept: 'application/json' } });
    ...
    async set(_key, value): Promise<void> {
      ...
      const response = await fetchImpl(endpoint(baseUrl), { credentials: 'include', method: 'PUT', headers, body: value });
