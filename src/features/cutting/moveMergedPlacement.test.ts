...
describe('movePlacementToBestOtherRoll', () => {
  ...
  it('does not move a piece when no other roll has an existing empty gap', () => {
    const requests = [request('a', 1210, 10000), request('b', 1210, 10000)];
    const rolls = requests.map((entry, index) => ({ placements: [{ id: index + 1, sourceId: `g1-${entry.pieceId}`, instanceIndex: 0, x: 5, y: 5, width: 1210, height: 10000, rotated: false }], usedLengthMm: 10010, producedQuantity: 1, utilizationPercent: 99, wastePercent: 1 }));
