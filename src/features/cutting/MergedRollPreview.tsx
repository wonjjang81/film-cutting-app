...
type Props = {
  ...
  onRotatePlacement?(placementId: number): string | null;
  ...
  automaticUtilizationPercent?: number;
  learnedLayoutApplied?: boolean;
...
export function MergedRollPreview({ plan, job, busy = false, onToggleComplete, onTogglePlacementComplete, onMovePlacementToAnotherRoll, onMovePlacementToNewRoll, onMovePlacementWithinRoll, onRotatePlacement, onShiftPlacementToEdge, onReoptimize, onDragAutoScroll, onDragGestureChange, canUndo = false, canRedo = false, canReset = false, onUndo, onRedo, onReset, selectedRollIndex: controlledRollIndex, onSelectRoll, hideHeading = false, hideRollTabs = false, hideControls = false, compact = false, hidePlacementList = false, hideLegend = false, continuousPageView = false, automaticUtilizationPercent, learnedLayoutApplied = false, completedPlacementIds: completedPlacementIdsOverride, sourceLabels, sourceSubgroups, sourceMajorGroups, collapsedSubgroups, onChangeCollapsedSubgroups }: Props) {
  ...
  const utilization = layoutUtilizationComparison(automaticUtilizationPercent ?? result.utilizationPercent, result.utilizationPercent);
  ...
  return (
    <View style={styles.wrap} accessibilityLabel={`병합 ${plan.mergeGroupId} 롤 미리보기`}>
      ...
      {learnedLayoutApplied && <Text style={styles.learnedBadge}>우수 수동 배치 학습 적용</Text>}
      <View style={styles.utilizationComparison} accessibilityLabel={`자동 배치 공간 활용 ${utilization.automatic}퍼센트, 수동 배치 공간 활용 ${utilization.current}퍼센트`}>
        <View style={styles.utilizationMetric}><Text style={styles.utilizationLabel}>자동 배치 · 고정</Text><Text style={styles.utilizationAutomatic}>{utilization.automatic}%</Text></View>
        ...
        <View style={styles.modalBackdrop}>
          {selected && (() => {
            ...
            return <View style={styles.modalCard} accessibilityViewIsModal accessibilityLabel="병합 조각 정보 팝업">
              ...
              {onShiftPlacementToEdge && <View style={styles.modalDirectionPad} accessibilityLabel="조각 상하좌우 끝 이동">
                <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${info.label} 위쪽 끝으로 이동`} disabled={busy} onPress={() => { const error = onShiftPlacementToEdge(selected.id, 'top'); setMoveError(error); if (!error) setSelectedId(null); }} style={[styles.modalDirectionButton, styles.modalDirectionTop, busy && styles.disabled]}><Text style={styles.modalDirectionText}>↑</Text></TouchableOpacity>
                <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${info.label} 왼쪽 끝으로 이동`} disabled={busy} onPress={() => { const error = onShiftPlacementToEdge(selected.id, 'left'); setMoveError(error); if (!error) setSelectedId(null); }} style={[styles.modalDirectionButton, styles.modalDirectionLeft, busy && styles.disabled]}><Text style={styles.modalDirectionText}>←</Text></TouchableOpacity>
                <View style={styles.modalDirectionCenter}><Text style={styles.modalDirectionCenterText}>끝 이동</Text></View>
                <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${info.label} 오른쪽 끝으로 이동`} disabled={busy} onPress={() => { const error = onShiftPlacementToEdge(selected.id, 'right'); setMoveError(error); if (!error) setSelectedId(null); }} style={[styles.modalDirectionButton, styles.modalDirectionRight, busy && styles.disabled]}><Text style={styles.modalDirectionText}>→</Text></TouchableOpacity>
              ...
              {onRotatePlacement && <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${info.label} 90도 수동 회전`} disabled={busy} onPress={() => { const error = onRotatePlacement(selected.id); setMoveError(error); if (!error) setSelectedId(null); }} style={[styles.modalMove, busy && styles.disabled]}><Text style={styles.modalMoveText}>↻ 90° 수동 회전</Text></TouchableOpacity>}
...
const styles = StyleSheet.create({
  ...
  learnedBadge: { alignSelf: 'flex-start', marginTop: 9, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999, overflow: 'hidden', fontSize: 9, fontWeight: '900', color: '#6d28d9', backgroundColor: '#ede9fe' },
  utilizationComparison: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 10, padding: 9, borderWidth: 1, borderColor: '#dbeafe', borderRadius: 9, backgroundColor: '#f8fafc' }, utilizationMetric: { flex: 1, minWidth: 88 }, utilizationLabel: { fontSize: 9, fontWeight: '800', color: '#64748b' }, utilizationAutomatic: { marginTop: 2, fontSize: 15, fontWeight: '900', color: '#2563eb' }, utilizationCurrent: { marginTop: 2, fontSize: 15, fontWeight: '900', color: '#0f766e' }, utilizationDelta: { minWidth: 48, textAlign: 'right', fontSize: 12, fontWeight: '900', color: '#64748b' }, utilizationDeltaUp: { color: '#15803d' }, utilizationDeltaDown: { color: '#dc2626' },
