...
type Props = {
  ...
  onMovePlacementToAnotherRoll?(placementId: number): void;
  compact?: boolean;
        ...
        <View style={styles.modalBackdrop}>
          {selected && (() => {
            ...
            return <View style={styles.modalCard} accessibilityViewIsModal accessibilityLabel="병합 조각 정보 팝업">
              ...
              {rolls.length > 1 && <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${info.label} 다른 롤 빈공간으로 자동배치`} disabled={busy || !onMovePlacementToAnotherRoll} onPress={() => { onMovePlacementToAnotherRoll?.(selected.id); setSelectedId(null); }} style={[styles.modalMove, (busy || !onMovePlacementToAnotherRoll) && styles.disabled]}><Text style={styles.modalMoveText}>다른 롤 빈공간으로 자동배치</Text></TouchableOpacity>}
