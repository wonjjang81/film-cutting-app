...
type Props = {
  ...
  onShiftPlacementHorizontally?(placementId: number, direction: 'left' | 'right'): string | null;
  onReoptimize?(): void;
        ...
        <View style={styles.modalBackdrop}>
          {selected && (() => {
            ...
            return <View style={styles.modalCard} accessibilityViewIsModal accessibilityLabel="병합 조각 정보 팝업">
              ...
              {onShiftPlacementHorizontally && <View style={styles.modalShiftRow} accessibilityLabel="조각 좌우 밀기">
                <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${info.label} 왼쪽으로 밀기`} disabled={busy} onPress={() => { const error = onShiftPlacementHorizontally(selected.id, 'left'); setMoveError(error); if (!error) setSelectedId(null); }} style={[styles.modalShift, busy && styles.disabled]}><Text style={styles.modalShiftText}>← 왼쪽 끝으로</Text></TouchableOpacity>
                <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${info.label} 오른쪽으로 밀기`} disabled={busy} onPress={() => { const error = onShiftPlacementHorizontally(selected.id, 'right'); setMoveError(error); if (!error) setSelectedId(null); }} style={[styles.modalShift, busy && styles.disabled]}><Text style={styles.modalShiftText}>오른쪽 끝으로 →</Text></TouchableOpacity>
              </View>}
