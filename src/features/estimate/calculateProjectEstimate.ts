import type { SavedCuttingJob } from '../library/models';
import { calculateEstimate, DEFAULT_CONSTRUCTION_COST_PER_M2, DEFAULT_MATERIAL_COST_PER_M, type Estimate } from './calculateEstimate';

export type ProjectEstimate = Estimate & { jobCount: number; jobs: Array<{ job: SavedCuttingJob; estimate: Estimate }> };

/** Sums saved jobs into one project invoice and applies the discount once to the total. */
export function calculateProjectEstimate(
  jobs: readonly SavedCuttingJob[],
  materialCostPerM = DEFAULT_MATERIAL_COST_PER_M,
  constructionCostPerM2 = DEFAULT_CONSTRUCTION_COST_PER_M2,
  discountRateOverride?: number,
): ProjectEstimate {
  const details = jobs.map((job) => ({ job, estimate: calculateEstimate(job, materialCostPerM, constructionCostPerM2, 0) }));
  const materialLengthM = details.reduce((sum, item) => sum + item.estimate.materialLengthM, 0);
  const materialCost = details.reduce((sum, item) => sum + item.estimate.materialCost, 0);
  const productAreaM2 = details.reduce((sum, item) => sum + item.estimate.productAreaM2, 0);
  const constructionCost = details.reduce((sum, item) => sum + item.estimate.constructionCost, 0);
  const subtotal = materialCost + constructionCost;
  const automaticDiscountRate = productAreaM2 >= 10 ? 0.15 : productAreaM2 >= 5 ? 0.1 : productAreaM2 >= 1 ? 0.05 : 0;
  const discountRate = discountRateOverride === undefined ? automaticDiscountRate : Math.min(1, Math.max(0, discountRateOverride));
  const discount = Math.max(0, Math.round(subtotal * discountRate));
  return { materialLengthM, materialCost, productAreaM2, constructionCost, subtotal, discountRate, discount, total: subtotal - discount, jobCount: jobs.length, jobs: details };
}
